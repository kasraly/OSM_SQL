# -*- coding: utf-8 -*-
"""
Created on Tue Dec 06 21:00:57 2016
...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    # YOUR CODE HERE
    if element.tag == 'node':
        elem_attribs = element.attrib
        for attrib in node_attr_fields:
            if attrib in ['id', 'uid', 'changeset']:
                node_attribs[attrib] = int(elem_attribs[attrib])
            elif attrib in ['lat', 'lon']:
                node_attribs[attrib] = float(elem_attribs[attrib])
            else:
                node_attribs[attrib] = elem_attribs[attrib]
                
        element_tags = element.iter('tag')
        for tag in element_tags:               
            tag_attribs = tag.attrib
            dic = {}
            dic['id'] = node_attribs['id']
            key = tag_attribs['k']
            value = tag_attribs['v'].strip()
    ...
    if element.tag == 'way':
        ...
        for attrib in way_attr_fields:
            if attrib in ['id', 'uid', 'changeset']:
                way_attribs[attrib] = int(elem_attribs[attrib])
            else:
                way_attribs[attrib] = elem_attribs[attrib]
        ...
        element_nds = element.iter('nd')
        for i, nd  in enumerate(element_nds):
            dic = {}
            dic['id'] = way_attribs['id']
            dic['node_id'] = int(nd.get('ref'))
            dic['position'] = i
...
def validate_element(element, validator, schema=SCHEMA):
    ...
    if validator.validate(element, schema) is not True:
        ...
        raise Exception(message_string.format(field, error_string, element))
